...
static FLaC__INLINE void *safe_malloc_(size_t size)
{
	...
	if(!size)
		size++;
	void * p;
	Serial.print("MALLOC");Serial.print(size);
	p = malloc(size);
	Serial.print("OK ");
	return p;
}
...
/* better? */
{
Serial.print("safe_malloc_mul_2op_ ");
	if(!size1 || !size2)
		return malloc(1); /* malloc(0) is undefined; FLAC src convention is to always allocate */
...
{
Serial.print("safe_malloc_muladd2_");Serial.print(size1 * (size2 + size3));
	if(!size1 || (!size2 && !size3))
		return malloc(1); /* malloc(0) is undefined; FLAC src convention is to always allocate */
	size2 += size3;
	if(size2 < size3)
		return 0;
		Serial.print("Call _mul_2op_");
...
{
Serial.print("_mul_2op");Serial.print(size1 * size2);
	if(!size1 || !size2)
